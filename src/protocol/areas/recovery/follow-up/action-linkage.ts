...
import type { RecoveryRecommendation, RecoveryRecommendedPath } from "../types";
...
export function assertRecoveryActionLinkage(
  ...
  const missingEvidence = recommendation.requiredNewEvidence.filter(
    (evidenceRef) => !input.evidenceRefs.includes(evidenceRef),
  );
