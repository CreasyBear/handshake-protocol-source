...
import { CreateRecoveryRecommendationInputSchema, type CreateRecoveryRecommendationInput } from "./types";
...
type ParsedCreateRecoveryRecommendationInput = ReturnType<typeof CreateRecoveryRecommendationInputSchema.parse>;

type RecoveryRecommendationContext = {
  input: ParsedCreateRecoveryRecommendationInput;
...
async function getRecoveryRecommendationContext(
  recorder: ProtocolRecorder,
  input: ParsedCreateRecoveryRecommendationInput,
): Promise<RecoveryRecommendationContext> {
...
async function buildRecoveryRecommendation(context: RecoveryRecommendationContext): Promise<RecoveryRecommendation> {
  ...
  return RecoveryRecommendationSchema.parse({
    ...
    safeRetryAvailable: safeRetryAvailable(
      input.recommendedPath,
      input.requiresHumanReview,
      input.allowedNextActionClasses,
    ),
...
function buildRecoveryRecommendationBinding(context: RecoveryRecommendationContext): JsonValue {
...
async function commitRecoveryRecommendation(
  recorder: ProtocolRecorder,
  context: RecoveryRecommendationContext,
  recommendation: RecoveryRecommendation,
): Promise<void> {
  await recorder.commitRecordsWithEvents(
    recoveryRecommendationRecords(recommendation),
    recoveryRecommendationEvents(context, recommendation),
  );
}

function recoveryRecommendationRecords(recommendation: RecoveryRecommendation): ProtocolRecord[] {
  return [{ objectType: "recovery_recommendation", payload: recommendation }];
}

function recoveryRecommendationEvents(
  context: RecoveryRecommendationContext,
  recommendation: RecoveryRecommendation,
): EventDescriptor[] {
  return [
...
function assertSourceRefBelongsToReceipt(receipt: Receipt, proofGaps: ProofGap[], sourceRefusalOrGapRef: string): void {
...
function assertProofGapBelongsToReceipt(receipt: Receipt, proofGap: ProofGap): void {
  const refs = new Set(
    [proofGap.receiptId, proofGap.gateAttemptId, proofGap.mutationAttemptId, ...proofGap.affectedObjectRefs].filter(
