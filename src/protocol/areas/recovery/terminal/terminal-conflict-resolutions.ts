...
import { ResolveRecoveryTerminalConflictInputSchema, type ResolveRecoveryTerminalConflictInput } from "../types";
...
type ParsedResolveRecoveryTerminalConflictInput = ReturnType<typeof ResolveRecoveryTerminalConflictInputSchema.parse>;

type RecoveryTerminalConflictResolutionContext = {
  input: ParsedResolveRecoveryTerminalConflictInput;
  proofGap: ProofGap;
  statusTransition: RecoveryRecommendationStatusTransition;
  recoveryRecommendation: RecoveryRecommendation;
...
async function getRecoveryTerminalConflictResolutionContext(
  recorder: ProtocolRecorder,
  input: ParsedResolveRecoveryTerminalConflictInput,
): Promise<RecoveryTerminalConflictResolutionContext> {
...
function buildRecoveryTerminalConflictResolution(
  context: RecoveryTerminalConflictResolutionContext,
...
async function commitRecoveryTerminalConflictResolution(
  recorder: ProtocolRecorder,
  context: RecoveryTerminalConflictResolutionContext,
  resolution: RecoveryTerminalConflictResolution,
): Promise<void> {
  await recorder.commitRecordsWithEvents(
    recoveryTerminalConflictResolutionRecords(resolution),
    recoveryTerminalConflictResolutionEvents(context, resolution),
  );
}

function recoveryTerminalConflictResolutionRecords(resolution: RecoveryTerminalConflictResolution): ProtocolRecord[] {
  return [{ objectType: "proof_gap", payload: resolution.proofGap }];
}

function recoveryTerminalConflictResolutionEvents(
  context: RecoveryTerminalConflictResolutionContext,
  resolution: RecoveryTerminalConflictResolution,
): EventDescriptor[] {
