...
export function guardActionProposal(input: {
  ...
  if (
    input.compilation.operatingEnvelopeId !== input.envelope.envelopeId ||
    input.envelopeId !== input.envelope.envelopeId
  ) {
    return fail(
      "invalid_transition_envelope_mismatch",
      "Action contract must use the envelope pinned by the intent compilation.",
    );
  }
  ...
  if (
    input.gateway.receiptCapabilityStatus !== "available" ||
    input.gateway.isolationCheckCapabilityStatus !== "available"
  ) {
    return fail(
      "gateway_not_enforcing",
      "Gateway registry entry does not prove receipt and isolation check capability.",
    );
  }
  if (
    ...
    return fail(
      "invalid_transition_scope_mismatch",
      "Action contract scope must match compilation, envelope, and gateway scope.",
    );
  }
  if (
    ...
    return fail(
      "invalid_transition_actor_mismatch",
      "Action contract actor bindings must match compilation and envelope.",
    );
  }
  return ok();
}
