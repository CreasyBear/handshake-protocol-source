...
import type { Greenlight, PolicyDecision } from "./types";
...
export async function loadSequenceDependencyStates(
  ...
  const policyDecisionsByActionContractId = indexByActionContractId(
    policyDecisionRecords.map((record) => record.payload),
  );
...
export function evaluateSequenceDependencies(states: SequenceDependencyState[]): PolicyEvaluationResult | null {
...
export async function loadGatewayCheckSequenceDependencyStates(
  ...
  const reconciliationsByMutationAttemptId = groupByMutationAttemptId(
    reconciliationRecords.map((record) => record.payload),
  );
  ...
  for (const requiredPriorActionContractId of contract.requiredPriorActionContractIds) {
    ...
    const finalPassedReceipt = receipts.find(
      (receipt) =>
        ...
        (receipt.finalityStatus === "final" || hasFinalReconciliation(receipt, reconciliationsByMutationAttemptId)),
    );
...
function groupByMutationAttemptId(
  records: SurfaceOperationReconciliation[],
): Map<string, SurfaceOperationReconciliation[]> {
