...
import {
  ...
  type CreateGeneratedExecutionGraphInputSchema,
...
export async function deriveGraphCoverage(input: ParsedGeneratedExecutionGraphInput): Promise<GraphCoverageEvaluation> {
...
function coverageStatusForGraph(
  ...
  if (
    nodes.length > 0 &&
    nodes.every((node) => node.classification === "read_only" || node.classification === "nonconsequential")
  ) {
  ...
  if (
    ...
    nodes.every((node) => ["candidate_action_eligible", "read_only", "nonconsequential"].includes(node.classification))
  ) {
