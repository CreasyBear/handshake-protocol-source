...
It turns vague principal intent and agent-generated orchestration into exact, inspectable, policy-evaluated, receiver-gated action contracts before consequential mutation.
...
- `src/protocol/action-contracts.ts`: exact receiver-bound contract proposal.
...
- `src/protocol/receiver-gate.ts`: receiver drift, finality, and status helpers.
- `src/protocol/receiver-gate-attempts.ts`: final receiver-gate enforcement before mutation.
- `src/protocol/receiver-gate-artifacts.ts`: structured gate attempts, mutation attempts, receipts, and events.
- `src/protocol/receiver-operation-reconciliations.ts`: same-mutation downstream reconciliation.
...
- `src/protocol/content-digests.ts`: deterministic UTF-8 content digests for content-bound receiver contracts.
...
- `src/adapters/package-install/receiver.ts`: reference package-install receiver adapter seam.
- `src/adapters/repo-write/receiver.ts`: reference repo-write receiver adapter seam.
...
POST /v0.2/receiver-operation-reconciliations
...
// 2. compileIntent(...) loads durable catalog/receiver records and records assumptions.
// 3. proposeActionContract(...) loads durable envelope/receiver records and only succeeds for a clean compilation.
// 4. evaluatePolicy(...) returns a greenlight or refusal.
// 5. receiverGate(...) consumes the greenlight before mutation.
// 6. receipt/proof-gap records reconstruct what happened.
// 7. createRecoveryRecommendation(...) can recommend a narrowed next contract after refusal or proof gap.
// 8. A later proposeActionContract(...) may link that recommendation, but still has no greenlight.
// 9. resolveRecoveryTerminalConflictProofGap(...) can close a terminal race gap after loading the winning transition.
```

The package-install receiver adapter demonstrates the receiver side of the loop:

```ts
import { runPackageInstallReceiver } from "./src";

await runPackageInstallReceiver({
  protocol: kernel,
...
The adapter mutates its manifest surface only after `receiverGate(...)` consumes the exact greenlight and the protocol derives a `VerifiedReceiverGateCheck`. If downstream finality is unknown, the gate records a `ProofGap`; the adapter still mutates only through the verified gate artifact and then reconciles that same mutation attempt by idempotency key and receiver operation ref.

The package-install runtime wrapper demonstrates the runtime side of the loop. It turns a generated package-install tool call into an `IntentCompilationRecord` and, only when catalog/receiver evidence is clean, an exact `ActionContract`. It does not evaluate policy, issue greenlights, call the receiver gate, or mutate a package manager.

`test/package-install-end-to-end.test.ts` connects the runtime wrapper, deterministic policy, and receiver adapter against a file-backed manifest. It proves the generated tool call leaves the manifest untouched until the exact contract receives a one-use greenlight and the receiver gate passes, then reconciles the same mutation attempt by idempotency key.

`test/d1-http.test.ts` proves the same package-install runtime and receiver adapters work through `HandshakeClient` against the Hono/D1 protocol surface. It covers the green path, a receiver parameter mismatch refusal where the manifest remains unchanged and D1 records no `MutationAttempt`, an unknown-finality path where a `ProofGap` is recorded and later resolved by reconciliation of the same mutation attempt, a recovery-linked follow-up `ActionContract` that records narrowed evidence without creating another mutation or greenlight, and a terminal-claim race that records a recovery-phase `ProofGap`.

`test/repo-write-d1-http.test.ts` proves the same protocol primitives against a second consequence shape: repository file writes. The runtime contract binds repository, file path, content digest, and byte length without putting raw file content in the `ActionContract`; the receiver recomputes the digest from actual content before the gate and refuses if generated content changed after proposal.

`test/codemode-multi-action-runtime.test.ts` proves a generated-program wrapper can emit ordered package-install and repo-write `ActionContract` proposals through Hono/D1 without receiving policy, greenlight, receiver-gate, or mutation authority. Later contracts carry `requiredPriorActionContractIds`; policy refuses them until each declared predecessor has a durable greenlight, and the receiver gate refuses them until each predecessor has a final passed receipt. A refused candidate records an `IntentCompilationRecord` with refusal reasons while already proposed contracts remain non-authoritative until policy and receiver gates run separately.

## Failure Modes Are First-Class

The kernel explicitly records:

- missing durable tool/action/receiver records as compilation uncertainty;
...
- receiver registry mismatches before contract emission;
- pinned receiver registry and policy versions on contracts and greenlights;
- receiver policy drift refusals at the gate unless the current receiver explicitly declares the older policy compatible and stricter;
...
- review decisions bound to exact contract digest and policy input digest;
- policy refusals, review requirements, halts, and quarantines;
- atomic record-plus-stream commits for intent compilation, action proposal, policy, review, isolation, and receiver-gate lifecycle writes;
- receiver gate refusals before mutation;
- receiver-operation reconciliation for pending or unknown downstream status without creating a second mutation attempt;
- one-use greenlight replay attempts;
- transactional receiver-gate commits for greenlight consumption, gate attempts, mutation/proof-gap, receipts, and stream events;
- receiver-gate stream offset conflicts as bounded retries against a fresh event tail;
...
- `greenlight_consumptions` as the one-use authority ledger;
...
Lifecycle writes use D1 `batch()` so protocol records and their stream events commit as one sequence. Receiver-gate writes additionally include greenlight consumption and the resulting gate/mutation/proof-gap/receipt records. If consumption loses a race, the kernel records a replay refusal without attempting mutation.

Stream events are chained per partition. Action lifecycle events emit into `action:{actionContractId}`, `run:{runId}`, and `receiver_resource:{receiverId}:{resourceRef}` partitions. Each partition starts at offset `0`, and each event carries the previous event digest. This makes proposal, policy, greenlight, gate, mutation/proof-gap, receipt, recovery recommendation, and recovery status transition replayable for one action, one generated run, or one receiver-owned resource. Receipts carry `streamOffsets` for those partitions, including terminal event digests, plus `receiptDigest` and `auditChainDigest`, so exported receipt evidence can find the exact event windows it summarizes and detect tampering. `ReceiptExport` verifies that digest material and packages a drop copy without creating execution proof. `RecoveryRecommendation` also verifies receipt digest material before it records a narrowed path, but it has `mustCreateNewActionContract: true`, `mayReuseGreenlight: false`, and `mayMutateReceiver: false`. A later `ActionContract` may carry `recoveryRecommendationId`, `recoverySourceReceiptId`, and `recoveryRecommendationDigest`; those fields bind evidence only and do not create policy or receiver authority. A linked proposal supersedes the recommendation in the same lifecycle commit through a `RecoveryRecommendationStatusTransition`, and `recovery_terminal_claims` rejects any concurrent second terminal transition. When a terminal claim loses a race, the kernel records a recovery-phase `ProofGap` on the source action stream and still returns `recovery_terminal_conflict`. Once the winning terminal transition is observed, the kernel may resolve only that proof gap by setting `resolvedByRef` to the terminal transition and emitting `proof_gap_resolved`; it does not retroactively persist the losing proposal. `IsolationState` can record `observedStreamOffsets` so a breaker-triggered quarantine is bound to the exact stream window it observed. If a receiver-gate commit loses an offset race, the kernel rebuilds the event chain from the fresh tail before retrying the atomic commit.

The test suite includes a D1-compatible Hono integration path that applies the migration, drives the public `/v0.2/*` routes through pending mutation reconciliation, and asserts contiguous stream offsets, digest predecessor links, one mutation attempt, and one reconciliation record. The package-install adapter path uses the same Hono/D1 surface through `HandshakeClient`, including a durable receiver refusal that records a gate attempt and receipt without creating a mutation attempt, an unknown-finality proof gap that is later resolved by receiver-operation reconciliation, a recovery-linked follow-up proposal that records no second mutation or greenlight, an explicit expired recovery status transition, and a terminal-claim race that records a recovery-phase proof gap. The repo-write adapter path proves the receiver-gate seam is not package-manager-specific by enforcing content digest binding before writing a file.

The compact completion audit is in `docs/protocol-completion-audit-v0.2.md`. It maps the plan obligations to current modules, routes, D1 evidence, runtime wrappers, receiver adapters, and invariant tests.
