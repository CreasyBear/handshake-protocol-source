...
describe("OpenClaw x402 protected tool activation artifact", () => {
  it("builds a host-specific OpenClaw tool-packet activation proof packet without creating authority", () => {
    const artifact = buildOpenClawX402ProtectedToolActivation(validActivationInput());
    ...
    expect(artifact).toMatchObject({
      ...
      manifest: {
        ...
        protectedPath: {
          configuredWrapperPath: "openclaw.tool-packet.json#toolPacket.handshake_x402_protected_tool",
          resolvedWrapperPath: "openclaw.tool-packet.json#toolPacket.handshake_x402_protected_tool",
        },
      },
      proofPacket: {
    ...
    expect(artifact.proofGaps).toContain("live_user_openclaw_tool_packet_write_not_performed_by_demo");
    expect(artifact.proofGaps).toContain("openclaw_host_wide_containment_not_claimed");
    expect(artifact.proofGaps).toContain("openclaw_native_host_behavior_not_claimed");
    expect(artifact.proofGaps).toContain("openclaw_unmanaged_tool_packet_control_not_claimed");
    expect(artifact.nonClaims).toContain("openclaw_activation_artifact_is_not_authorization");
    expect(artifact.nonClaims).toContain("openclaw_activation_artifact_is_not_native_host_certification");
    expect(artifact.nonClaims).toContain("openclaw_activation_artifact_is_not_host_wide_containment");
  ...
  it("records unmanaged tool-packet mutation possibility as not-ready host posture", () => {
    const artifact = buildOpenClawX402ProtectedToolActivation(
        ...
        rawSiblingResults: [
          {
            ...
            evidenceRefs: ["evidence:openclaw-x402-activation:unmanaged-tool-packet-mutated"],
            proofGapReasonCodes: [],
          },
        ],
      }),
...
function validActivationInput(
  overrides: Partial<OpenClawX402ProtectedToolActivationInput> = {},
): OpenClawX402ProtectedToolActivationInput {
  return {
    activationId: "activation_openclaw_x402_tool_packet",
    packageIdentifier: "handshake-protocol-kernel",
...
function validProfileInput(): ProtectedX402ToolHostProfileInput {
  return {
    profileId: "profile_x402_openclaw",
    hostFamily: "openclaw",
    hostProfileRef: "host-profile:openclaw:x402",
    runtimeAdapterRef: "runtime-adapter:openclaw",
    toolCatalogDigest: digest(11),
    metadataDigest: digest(12),
    gatewayReadiness: validGatewayReadiness(),
    facadeInput: validFacadeInput(),
    transcriptRef: "transcript:openclaw:x402",
    evidenceRefs: ["evidence:host-profile:openclaw"],
  };
}
