...
describe("Hermes x402 protected tool activation artifact", () => {
  it("builds a host-specific Hermes tool-packet activation proof packet without creating authority", () => {
    const artifact = buildHermesX402ProtectedToolActivation(validActivationInput());
    ...
    expect(artifact).toMatchObject({
      ...
      manifest: {
        ...
        protectedPath: {
          configuredWrapperPath: "hermes.tool-packet.json#toolPacket.handshake_x402_protected_tool",
          resolvedWrapperPath: "hermes.tool-packet.json#toolPacket.handshake_x402_protected_tool",
        },
      },
      proofPacket: {
    ...
    expect(artifact.proofGaps).toContain("live_user_hermes_tool_packet_write_not_performed_by_demo");
    expect(artifact.proofGaps).toContain("hermes_host_wide_containment_not_claimed");
    expect(artifact.proofGaps).toContain("hermes_native_host_behavior_not_claimed");
    expect(artifact.proofGaps).toContain("hermes_unmanaged_tool_packet_control_not_claimed");
    expect(artifact.nonClaims).toContain("hermes_activation_artifact_is_not_authorization");
    expect(artifact.nonClaims).toContain("hermes_activation_artifact_is_not_native_host_certification");
    expect(artifact.nonClaims).toContain("hermes_activation_artifact_is_not_host_wide_containment");
  ...
  it("records unmanaged tool-packet mutation possibility as not-ready host posture", () => {
    const artifact = buildHermesX402ProtectedToolActivation(
        ...
        rawSiblingResults: [
          {
            ...
            evidenceRefs: ["evidence:hermes-x402-activation:unmanaged-tool-packet-mutated"],
            proofGapReasonCodes: [],
          },
        ],
      }),
...
function validActivationInput(
  overrides: Partial<HermesX402ProtectedToolActivationInput> = {},
): HermesX402ProtectedToolActivationInput {
  return {
    activationId: "activation_hermes_x402_tool_packet",
    packageIdentifier: "handshake-protocol-kernel",
...
function validProfileInput(): ProtectedX402ToolHostProfileInput {
  return {
    profileId: "profile_x402_hermes",
    hostFamily: "hermes",
    hostProfileRef: "host-profile:hermes:x402",
    runtimeAdapterRef: "runtime-adapter:hermes",
    toolCatalogDigest: digest(11),
    metadataDigest: digest(12),
    gatewayReadiness: validGatewayReadiness(),
    facadeInput: validFacadeInput(),
    transcriptRef: "transcript:hermes:x402",
    evidenceRefs: ["evidence:host-profile:hermes"],
  };
}
