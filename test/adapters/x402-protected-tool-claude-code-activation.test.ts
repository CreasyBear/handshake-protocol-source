...
describe("Claude Code x402 protected tool activation artifact", () => {
  it("builds a host-specific Claude Code managed MCP activation proof packet without creating authority", () => {
    const artifact = buildClaudeCodeX402ProtectedToolActivation(validActivationInput());
    ...
    expect(artifact.proofGaps).toContain("live_user_claude_code_config_write_not_performed_by_demo");
    expect(artifact.proofGaps).toContain("claude_code_host_wide_containment_not_claimed");
    expect(artifact.proofGaps).toContain("claude_code_unmanaged_mcp_server_control_not_claimed");
    expect(artifact.nonClaims).toContain("claude_code_activation_artifact_is_not_authorization");
    expect(artifact.nonClaims).toContain("claude_code_activation_artifact_is_not_host_wide_containment");
  ...
  it("records unmanaged MCP mutation possibility as not-ready host posture", () => {
    const artifact = buildClaudeCodeX402ProtectedToolActivation(
        ...
        rawSiblingResults: [
          {
            ...
            evidenceRefs: ["evidence:claude-code-x402-activation:unmanaged-mcp-mutated"],
            proofGapReasonCodes: [],
          },
        ],
      }),
...
function validActivationInput(
  overrides: Partial<ClaudeCodeX402ProtectedToolActivationInput> = {},
): ClaudeCodeX402ProtectedToolActivationInput {
  return {
    activationId: "activation_claude_code_x402_managed_mcp",
    packageIdentifier: "handshake-protocol-kernel",
...
function validProfileInput(): ProtectedX402ToolHostProfileInput {
  return {
    profileId: "profile_x402_claude_code_mcp",
    hostFamily: "claude_code_mcp",
    hostProfileRef: "host-profile:claude_code_mcp:x402",
    runtimeAdapterRef: "runtime-adapter:claude_code_mcp",
    toolCatalogDigest: digest(11),
    metadataDigest: digest(12),
    gatewayReadiness: validGatewayReadiness(),
    facadeInput: validFacadeInput(),
    transcriptRef: "transcript:claude_code_mcp:x402",
    evidenceRefs: ["evidence:host-profile:claude_code_mcp"],
  };
}
