...
import type { HandshakeAmbiguousCommitError } from "../../src/protocol/foundation/errors";
...
describe("FaultInjectingProtocolStore", () => {
  ...
  it("can report ambiguous applied commits for retry-safety tests", async () => {
    const protocolStore = new FaultInjectingProtocolStore(
      new InMemoryProtocolStore(),
    ...
    const gatewayStore = new FaultInjectingProtocolStore(
      new InMemoryProtocolStore(),
    ).injectAmbiguousGatewayCommitOnce();
    await expect(gatewayStore.commitGatewayCheck({ consumption: null, records: [], events: [] })).rejects.toMatchObject(
      {
        code: "ambiguous_commit",
        metadata: { retryability: "ambiguous", commitState: "unknown" },
      } satisfies Partial<HandshakeAmbiguousCommitError>,
