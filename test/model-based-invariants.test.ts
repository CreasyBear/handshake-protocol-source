...
import type { GatewayCheckResult } from "../src/protocol/gateway-gate";
...
import { transitionMatrixByRouteId } from "./support/transition-matrix";
...
describe("model-based foundation invariants", () => {
  it("ties model commands to transition-matrix invariant rows", () => {
    const modeledRoutes = new Set(
      Object.values(modelCommands).flatMap((command) => command.routeIds),
    );

    for (const routeId of modeledRoutes) {
      expect(transitionMatrixByRouteId[routeId].invariantTests.length).toBeGreaterThan(0);
    }
...
async function assertStoreInvariants(context: ModelContext, label: string): Promise<void> {
  ...
  if (mutations.length !== context.expectedMutationAttempts) {
    throw new Error(`${label}: expected ${context.expectedMutationAttempts} mutation attempts, saw ${mutations.length}`);
  }
