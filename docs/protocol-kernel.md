...
Version: v0.2.0  
Audience: Protocol implementers, runtime builders, receiver owners, platform engineering  
...
`ReceiverRegistryEntry` binds a receiver ID to an adapter, receiver policy version, drift semantics, resource namespace, gate endpoint, receipt capability, and isolation-check capability.
...
`Greenlight` binds one policy decision to one exact contract and one receiver-gated attempt. It has `maxUses: 1`.

`ReceiverGateAttempt` records the receiver-side check before mutation, including pinned vs current receiver policy version and drift status.

`MutationAttempt` records whether the receiver attempted downstream work after the gate passed.

`ReceiverOperationReconciliation` records same-operation status checks after a pending or unknown downstream outcome. It binds to the original mutation attempt and idempotency key; it does not authorize another mutation.
...
`Receipt` reconstructs the execution chain without pretending that receiver check equals downstream success. It carries `streamOffsets` for the action, run, and receiver-resource partitions it summarizes, plus `receiptDigest` and `auditChainDigest` for tamper-evident export.
...
`IsolationState` is durable interdict state checked before future policy decisions and receiver gates. It can carry `observedStreamOffsets` so a breaker-triggered isolation records the exact stream partitions, offsets, and tail digests it observed.
...
- `receiver_resource:{receiverId}:{resourceRef}` reconstructs ordering against one receiver-owned resource.

Lifecycle stream appends are conflict-aware. Intent compilation, action proposal, policy decisions, greenlights, review decisions, isolation state, and receiver-gate records commit with their stream events. If the D1 write detects that another event already claimed the planned offset, the kernel rebuilds the event chain from the new tail and retries the atomic commit. A protocol object may not persist as authoritative evidence without its stream event.

The reference implementation keeps primitive logic in separate modules: durable object schemas, request input schemas, intent compilation, action contract proposal, policy decisions, review decisions, isolation state writes, transition guards, receiver-gate enforcement, receiver-operation reconciliation, proof gaps, durable recording, and stream event construction are independent of the Hono routes and storage adapters. `HandshakeKernel` is a façade over these modules and the `ProtocolStore` interface; it should not absorb receiver-specific policy, runtime plugin logic, or product UI state.

The package-install reference flow now exercises this separation end to end: a runtime wrapper may compile and propose, policy may greenlight, and a receiver adapter may mutate only after the receiver gate consumes the exact greenlight and yields a `VerifiedReceiverGateCheck`. The tests assert the external manifest is unchanged before the gate and that the action stream records proposal, policy, greenlight, gate, mutation, receipt, and reconciliation in digest-linked order. The Hono/D1 path proves the same chain through `HandshakeClient`, including a parameter-mismatch receiver refusal with no mutation attempt and an unknown-finality proof gap later resolved by reconciliation of the same mutation attempt.

The recovery recommendation path exercises the post-failure boundary. A proof-gap receipt can produce a `RecoveryRecommendation` on the same action/run/resource stream, but a final successful receipt cannot. A later `ActionContract` can link to that recommendation only when it matches scope, sequence, allowed action class, timing, and required new evidence. The linked proposal supersedes the recommendation in the same lifecycle commit. If a stale competing follow-up loses the terminal claim race, the kernel records a recovery-phase proof gap; once the winning transition is observed, that gap can be resolved without retroactively persisting the loser. The recommendation creates no greenlight, no receiver gate attempt, and no mutation attempt.

The repo-write reference flow exercises the same primitive against a different consequence shape. It binds repository file content by digest and byte length, omits raw content from the `ActionContract`, recomputes the digest at the receiver adapter, and refuses mutation when actual content diverges from the contract.

The codemode multi-action reference flow exercises generated orchestration. It emits ordered package-install and repo-write contract proposals through the same Hono/D1 protocol surface, records candidate refusals as compiler evidence, binds later contracts to prior contract IDs, and proves the runtime wrapper receives no policy, greenlight, receiver-gate, or mutation authority. Policy refuses a later contract while any required predecessor is missing, refused, or not greenlit. The receiver gate refuses a later contract while any required predecessor lacks a final passed receipt.
...
| clean intent compilation -> action proposed | Compilation, envelope, receiver, principal, agent, run, tenant, and organization bindings must match. |
...
| greenlight -> receiver gate | Gate authority must come from a greenlight policy decision. Binding mismatches and unsatisfied sequence dependencies become receiver refusals before mutation. |
| pending/unknown mutation -> receiver operation reconciliation | Reconciliation must bind to the same mutation attempt and idempotency key. |
| refusal/proof-gap receipt -> recovery recommendation | Recovery may recommend a narrowed future contract or review path only. It cannot reuse a greenlight or mutate receiver state. |
...
If the receiver does not check the greenlight before mutation, this is advisory, not Handshake.

If one greenlight can authorize more than one mutation attempt, this is ambient authority wearing a badge.

If the receipt cannot distinguish receiver check from downstream execution, this is evidence theatre.

## Determinism

Contracts must be canonicalized deterministically. Policy must evaluate the canonical contract, current isolation state, and declared sequence dependencies. Receiver gates must compare the exact contract digest, receiver registry entry, receiver policy drift status, receiver ID, action class, resource, parameter digest, freshness window, isolation snapshot, greenlight consumption state, and prior final receipt state.
