...
Moment of value: a coding agent attempts one consequential action, Handshake turns it into an exact action contract, policy greenlights or refuses it, the receiver gate enforces before mutation, and the receipt timeline reconstructs what happened, what was refused, and what evidence is missing.
...
The full agentic experience exists to make that invariant feel obvious, usable, and valuable to a developer team. If the experience is beautiful but cannot point to exact protocol objects and receiver-side enforcement, it is product theatre.

## Experience Thesis

Agents may propose and orchestrate. Handshake contracts. Receivers enforce. Receipts reconstruct.

The product is not a dashboard around agent activity. The product is the path from vague intent to receiver-gated consequence:
...
The experience should feel as legible as a Vercel preview deploy, as precise as a Stripe API call, as fast as a Cloudflare Worker, as disciplined as a Linear issue state, and as dependency-aware as installing from npm. The difference is that every visual and API surface must bind to receiver-enforced execution authority.
...
```text
I would let an agent do this larger action because the receiver refuses anything outside the exact contract, and I can reconstruct the chain afterward.
```

That requires five product proofs:

1. Activation: reach the first receiver-gated receipt quickly.
2. Authority: greenlight is exact, one-use, receiver-bound, and consumed at the gate.
3. Refusal: missing, replayed, mismatched, isolated, drifted, or evidence-unavailable attempts refuse before mutation.
4. Reconstruction: timeline separates policy decision, receiver check, mutation attempt, downstream status, proof gap, reconciliation, and isolation.
...
|---|---|---|
| Exact binding | The surface shows or carries `action_contract_id`, canonical digest, receiver ID, action class, resource, parameter digest, and policy/receiver versions where relevant. | A user can approve or execute from a summary that differs from the contract. |
| Receiver primacy | The receiver gate is the enforcement boundary before mutation. | The product claims control from a runtime hook, plugin, or log. |
...
| Isolation is enforceable | Isolation is checked at policy time and receiver-gate time. | A stale greenlight can mutate after quarantine. |
...
Handshake is not an omniscient observer over every shell, browser, network, MCP, or generated-code path. It works when a consequential action is forced through a protected action path or when the receiver refuses raw calls that lack an exact current greenlight.
...
|---|---|---|
| Hook-assisted runtime | Better classification and refusal UX inside runtimes with before-tool-call hooks | Receiver gate, not the hook |
| Protected MCP/CLI capability | Default install path for Codex, Claude Code, MCP, and CLI-shaped workflows | Receiver adapter owning credentials plus receiver gate |
| Codemode block | Generated programs declare tools, receivers, retry limits, branches, and evidence expectations | Each consequential branch still needs an exact contract and gate |
| Receiver-only enforcement | Minimal mode for thin runtimes or hostile environments | Receiver refuses anything without exact greenlight; missing runtime evidence becomes proof gap |

Raw credential rule: if the agent can mutate the receiver directly, Handshake does not control that action. The honest product state is bypass evidence or proof gap when visible, not receiver-gated control.
...
| Issuer runtime | Observes protected or declared tool attempts, classifies capability, records compilation evidence, and blocks local dispatch when required. | Receiver enforcement boundary or omniscient observer. |
| Handshake kernel | Canonicalizes contracts, evaluates policy, issues refusals/greenlights, records stream events, receipts, proof gaps, and isolation. | Receiver business logic. |
| Policy | Decides against exact contracts, envelopes, receiver policy, and current isolation. | Execution proof. |
| Receiver | Owns consequence and enforces exact greenlight before mutation. | Passive log sink. |
...
- Generated execution block bounds may frame allowed tools, receivers, retry limits, expiry, evidence, and stop conditions.
...
- `ActionType` and `ReceiverRegistryEntry` must be pinned before a contract can exist.
...
- `IntentCompilationRecord` records candidate actions, assumptions, overreach, uncertainty markers, generated code/spec references, catalog versions, and receiver policy versions.
...
- `ActionContract` binds receiver, resource, parameter digest, side effects, bounds, idempotency key, canonicalizer version, and canonical digest.
...
- `PolicyDecision` evaluates the exact contract against the envelope, policy pack, receiver policy, and current `IsolationState`.
...
### 7. Receiver Gate Enforces

User-visible moment:

```text
Receiver checks the greenlight before mutation.
```

Mechanism:

- `ReceiverGateAttempt` verifies greenlight existence, freshness, one-use state, contract digest, receiver ID, action class, resource reference, parameter digest, receiver policy drift, idempotency, isolation, and receipt/outbox capability.
- Greenlight consumption must be atomic with gate evidence.

Experience rule: if the receiver does not refuse before mutation, the integration is advisory, not Handshake.
...
```text
Preview deployed, receiver refused, or evidence missing.
```

Mechanism:

- `MutationAttempt` records attempted downstream work only after the gate passes.
- Receiver refusal records why no mutation occurred.
...
```text
The run shows intent, contract, decision, greenlight, receiver check, mutation/refusal, proof gaps, reconciliation, and isolation.
```
...
- Reconciliation checks status for the same receiver operation; it does not create new authority.
...
- `IsolationState` interdicts future policy decisions and receiver gates.
...
- local approval as receiver execution proof.
...
- receiver ID and policy version;
- action class;
- resource reference;
...
Purpose: make receiver enforcement easy to install correctly.
...
- receiver policy drift semantics;
...
|---|---|
| Kernel API | Create compilation, contract, policy decision, receiver gate attempt, reconciliation, isolation, receipt lookup. |
| Runtime SDK | Classify tool calls, emit compilation record, block/refuse ambiguity, pass greenlight reference to receiver-bound tool path. |
| Receiver SDK | Verify exact greenlight, consume once, record refusal/mutation/proof gap, append stream event. |
| Fixture runner | Run the golden path, refusal path, replay path, mismatch path, proof-gap path, and isolation race path. |
| Receipt renderer | Render one action chain from protocol objects. |
| Test harness | Assert forbidden transitions and receiver refusal before mutation. |
...
| Runtime adapter | Classify protected tool attempts, record compilation evidence, and return structured refusal or contract reference | Package-install and repo-write wrappers | Generated orchestration cannot reach policy or receiver-gate authority through this seam |
| Protected capability | MCP/CLI/tool surface exposed to agents instead of raw receiver tools | First demo capability for protected write or package install | Callers see one simple action while credentials and gate mechanics stay behind the seam |
| Receiver adapter | Verify exact greenlight, consume once, mutate only through verified gate artifact, reconcile downstream finality | Package-install and repo-write receivers | Receiver owners integrate one gate interface instead of duplicating policy logic |
...
Do not create product features that bypass these seams. If a surface cannot name its protocol module, interface, adapter, and receiver dependency, it is product theatre.
...
```text
T+0:00  install or run local demo
T+1:00  agent/fixture proposes one guarded action
T+2:00  exact contract and policy decision appear
T+3:00  receiver gate passes or refuses before mutation
T+4:00  receipt timeline reconstructs the chain
T+5:00  replay or parameter mismatch refuses before mutation
```

The exact CLI name can change, but the output contract cannot. The first run must print or render:

- action contract ID and canonical digest;
- policy decision and greenlight/refusal;
- receiver gate attempt outcome;
- mutation attempt, receiver refusal, or proof gap;
- receipt timeline URL or local render path;
- one recovery hint for refusal.

If the first developer journey cannot reach a receiver-gated receipt in one terminal session, the product is too abstract.
...
- pin action and receiver catalog versions before contract issuance;
...
- publish accepted contract shape and receiver policy version;
...
The business wedge can be "preview deploys without deployment authority" because buyers understand it quickly. The technical proof may use protected repo write or package install first because the current code already proves receiver-gated mutation, replay refusal, parameter mismatch refusal, proof gaps, and receipt reconstruction.

Do not demo preview deploy as controlled until a preview-deploy receiver adapter consumes exact greenlights before deployment. Until then, preview deploy is a sales narrative and target integration, not a proven receiver.

### Application 1: OpenClaw MCP-Style Protected Write

Why first: it is the smallest falsifiable runtime proof.
...
- before-tool-call observes tool and args;
- issuer adapter classifies capability;
- exact action contract is created;
- policy greenlights or refuses;
- receiver refuses missing/replayed/mismatched/isolated/drifted greenlights;
- receipt timeline reconstructs the run.
...
- raw terminal/network/filesystem/browser bypass is refused at receiver when possible and recorded as bypass evidence when observable.
...
- receipt distinguishes proposed diff, receiver gate, and actual write.
...
|---|---|
| Golden path | One exact action can be contracted, greenlit, receiver-gated, executed, and receipted. |
| Policy refusal | Refusal is understandable and recoverable before receiver mutation. |
| Receiver refusal | Missing, replayed, mismatched, isolated, or drifted greenlights refuse before mutation. |
| Bypass attempt | Direct receiver call without greenlight fails at the receiver. |
...
- Receiver gate before any management UI.
...
- Design-partner pilots scoped to one runtime, one receiver, one action.
...
- Receipt that collapses receiver check and downstream success.
- Proof gap hidden inside prose.
- Browser or shell control claim without receiver enforcement.
...
- the receiver gate is the only enforcement boundary for receiver-owned consequence;
...
- OpenClaw protected write is first, Hermes/codemode portability second, and preview deploy/protected repo write/package install/CI mutation are named expansion applications;
...
- it grants mutation authority outside a one-use receiver-bound greenlight;
...
- it cannot represent receiver refusal before mutation;
- it hides proof gaps;
...
- number of steps to install one receiver gate;
...
- proof-gap rate by receiver/action family;
- receiver refusal before mutation coverage;
...
Ship one design-partner activation packet:

```text
OpenClaw MCP-style protected write
  -> exact contract fixture
  -> one policy file
  -> receiver gate harness
  -> refusal tests
  -> receipt timeline render
  -> five-minute demo script
  -> buyer scorecard
```

The activation packet is complete only when a missing, replayed, mismatched, isolated, or receiver-policy-drifted greenlight refuses before mutation and the receipt timeline reconstructs the chain.
