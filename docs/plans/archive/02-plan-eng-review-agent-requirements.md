...
It treated agents too much like ordinary API clients or plugin users. That is not enough. Agents compile vague intent into tool calls, generated scripts, MCP operations, browser actions, scheduled jobs, repo writes, package installs, cloud mutations, and gateway-to-gateway workflows.

Generated orchestration may be useful. It is not authority.

Runtime hooks may block local dispatch. They are not gateway enforcement.
...
2. Gateway middleware alone proves only the downstream half. It does not prove that generated agent orchestration cannot bypass the contract boundary before reaching the gateway.
3. A scoped decision object that can float without an exact action contract becomes ambient authority wearing a badge.
4. Verify-only compatibility paths are not Handshake. If a gateway can mutate after token verification without checking the exact contract and one-use greenlight, the path is advisory.
5. Demo surfaces drift unless the refusal cases are first-class. Missing, replayed, mismatched, isolated, and gateway-policy-drifted greenlights must refuse before mutation in the first proof.
...
greenlight = one-use gateway-bound execution authority
gateway check = enforcement boundary before mutation
...
1. Generated execution block bounds. Codemode-style generated scripts may need parent bounds covering allowed tools, gateways, resources, side-effect classes, retry limits, stop conditions, freshness, and evidence expectations. These bounds are compilation context, not authority. Every consequential mutation inside the block still requires its own action contract unless represented as an explicit bounded batch contract.
2. Contract-stream isolation. Gateway gates and policy decisions must observe current isolation state. A listener can detect divergence and update isolation, but the gateway check must enforce it before mutation.
3. Gateway refusal tests. The first proof must show refusal for missing greenlight, replayed greenlight, mismatched contract digest, isolated subject/resource, gateway policy drift, and unavailable receipt/proof-gap evidence.

This is the narrow version of the research. Anything broader belongs outside v0.2 until the first gateway-checked loop works.

## Agent Requirement Thesis

Agents may propose and orchestrate. Gateways enforce. Handshake is the contract exchange between them.

The issuer side must reduce an agent-originated action attempt into catalog-bound contract candidates. The gateway side must publish what it will honor and enforce the exact greenlight before mutation. Handshake clears, refuses, records, and reconstructs the chain between those sides.

The protocol center remains the same primitive loop from the architecture review:

```text
agent-originated action attempt
  -> ToolCapability / ActionType / GatewayRegistryEntry lookup
  -> IntentCompilationRecord
  -> ActionContract
  -> PolicyDecision
  -> one-use Greenlight or refusal/review/halt/quarantine
  -> GatewayCheckAttempt
  -> MutationAttempt or gateway refusal
  -> Receipt or ProofGap
...
| Runtime and gateway agree on consequential categories | `ActionType` | Prevent model-invented action classes from becoming contracts. |
| Gateway publishes enforceable endpoint and accepted shape | `GatewayRegistryEntry`, `GatewayPolicyContract` | Bind gateway ID, gate endpoint, policy version, resource namespace, drift behavior, and evidence expectations. |
...
| Generated execution block bounds constrain codemode context | `IntentCompilationRecord`, `ActionContract`, `PolicyDecision`, `ContractStreamEvent` | Record allowed tools, gateways, resources, retry bounds, stop conditions, expiry, and evidence expectations as context; never treat block bounds as mutation authority. |
| Exact proposed consequence is normalized | `ActionContract` | Bind gateway, resource, parameters digest, non-secret summary, side effects, bounds, canonicalizer version, and idempotency key. |
| Policy clears or refuses the exact contract | `PolicyDecision` | Decide against the contract, envelope, gateway policy, and current isolation state. |
| Execution authority is one-use and gateway-bound | `Greenlight` | Bind one decision to one exact gateway-checked attempt; default `max_uses` is exactly `1`. |
| Gateway checks before mutation | `GatewayCheckAttempt` | Verify exact binding, freshness, isolation, gateway policy drift, idempotency, and greenlight consumption before consequence. |
| Gateway outcome is separated by phase | `MutationAttempt`, `Receipt`, `ProofGap` | Distinguish gate check, mutation attempt, downstream finality, refusal, and missing evidence. |
| Divergence blocks future consequence | `IsolationState`, `ContractStreamEvent` | Isolation must be checked at both policy decision time and gateway check time. |
| Human review, when required, is exact | `ReviewDecision` | Bind reviewer decision to contract digest, policy input digest, gateway policy version, render schema version, expiry, and attestation. |
...
- pin the `GatewayRegistryEntry` and gateway policy version used during compilation;
...
- record uncertainty markers when tool, gateway, resource, or parameter meaning is unclear;
...
- record issuer evidence as evidence contribution, not as gateway execution proof.

The issuer side may stop a local tool call. That is useful. It is still not the enforcement boundary for gateway-owned consequence.
...
OpenClaw generated tool call
  ...
  -> ActionContract binds gateway/action/resource/params digest
...
The proof target should use a fake but consequential gateway such as `crm.update_customer`, `github.create_issue`, or `repo.write_file`. The gateway must refuse missing, replayed, mismatched, isolated, and gateway-policy-drifted greenlights before mutation.

The proof must also emit a contract-stream event for every policy decision, greenlight, gateway refusal, mutation attempt, receipt, proof gap, and isolation transition needed to reconstruct the run. If stream ordering cannot show whether isolation predated a gateway check, the proof is incomplete.

Hermes becomes the second proof when the same contract exchange can be expressed through its hook model without weakening the issuer/gateway boundary.

## Gateway-Side Agent Requirements

The gateway side is the endpoint, service, gateway agent, MCP server, deployment surface, package manager adapter, repo writer, cloud mutation API, database operation layer, or other system that owns the consequence.

A gateway integration must publish:

- gateway ID and gateway owner;
- gateway kind and adapter version;
...
- gateway ID, action class, resource reference, and parameter digest match;
- gateway policy version is same-version or explicitly compatible stricter;
...
- gateway check evidence can be appended with ordering sufficient to reconstruct races against isolation changes.

If any check fails, the gateway must refuse before mutation.

## Generated Orchestration Requirements

Agent-generated orchestration includes codemode scripts, MCP tool sequences, generated JSON plans, browser-side tool calls, scheduled jobs, terminal commands, package operations, repo writes, cloud API calls, and gateway-to-gateway action chains.
...
- allowed gateway IDs and resource namespaces;
...
- `ambiguous`: consequence depends on parameters, gateway state, hidden tool behavior, or generated code path;
- `consequential`: mutation path observed or declared;
- `unwrapped_bypass`: consequential path can avoid the issuer adapter or gateway check.
...
- retries may reconcile the same surface operation only when bound to the same `mutation_attempt_id` and idempotency key;
...
- browser-side tools must be treated as bypass-prone unless their gateway check is observable and enforceable;
...
[intent_compiling]
      ...
      +-- unknown gateway/action --------> [compilation_refused]
      |
      v
[candidate_contract]
      |
...
[contract_stream_event_appended]
      |
      v
[gateway_check_checking]
      |
      +-- missing/mismatched/replayed ----> [gateway_refused_before_mutation]
      +-- current isolation --------------> [gateway_refused_before_mutation]
      +-- gateway policy drift ----------> [gateway_refused_before_mutation]
      |
      v
[greenlight_consumed]
      |
...
plugin_check -> gateway_execution_proof
issuer_receipt -> downstream_success
operating_envelope -> gateway_check_passed
```
...
|---|---|---|
| Official before/after tool hooks | Keep | Hooks produce issuer evidence and contract candidates; they do not enforce gateway-owned consequence. |
...
| Post-call receipt emission | Keep with boundary | Issuer evidence contribution only; gateway receipt remains canonical for consequence. |
...
| `intentCategory` | Rename | Use `ActionType.action_class` or gateway policy category. |
| `passport` | Defer or narrow | Identity evidence only; never mutation authority. |
| Plugin-only control claim | Reject | If the gateway does not enforce, the system is advisory, not Handshake. |
...
- plugin-only enforcement for gateway-owned consequence;
...
- operating envelope as gateway permission;
...
- it names both sides of the protocol: issuer agent/runtime and gateway endpoint/agent;
- it requires `IntentCompilationRecord` before `ActionContract`;
- it requires gateway policy contract publication before gateway-checked mutation;
...
- it makes issuer evidence and gateway receipts distinct;
- it requires refusal tests for missing, replayed, mismatched, isolated, and gateway-policy-drifted greenlights.
...
- gateway owners cannot tell what they must publish and enforce;
...
- missing gateway evidence can be hidden inside receipt prose;
- the first MCP write proof can mutate without gateway-side gate enforcement;
- stream ordering cannot reconstruct whether isolation happened before or after gateway check evaluation.
...
stream/isolation:
  ...
  current IsolationState check at gateway check time
  ordered refusal/mutation/receipt/proof-gap evidence
```

The proof is not complete until a missing, mismatched, replayed, isolated, or gateway-policy-drifted greenlight refuses before mutation.
