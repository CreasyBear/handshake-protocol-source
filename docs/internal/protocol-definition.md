...
Last protocol definition audit: 2026-05-19.
...
5. The `Greenlight` is unexpired, unconsumed, and has `maxUses: 1`.
6. No active `IsolationState` blocks policy or gateway execution.
7. The gateway owns or controls the mutation credential.
8. The gateway checks the exact greenlight before mutation.
