...
Active next product shipment: render the Contract Viewer and Receipt Timeline from the Handshake CLI/MCP protected action surface. The first adapter-backed timeline should use protected repo-write-to-PR and show the exact contract, policy decision, gateway check, generated branch/ref operation, exact commit, pull request evidence, downstream branch-protection state, and proof gaps as separate rows. Existing package-install or repo-write reference runs may supply conformance evidence, but they are not the product path.
