...
|---|---:|---|
| no action contract without pinned tool/action/gateway catalog bindings | Pass | `src/protocol/intent-compilation.ts`, `src/protocol/action-contracts.ts`, `test/kernel.test.ts` catalog and gateway mismatch cases, `test/package-install-runtime.test.ts` unknown catalog refusal |
...
| no gateway check without exact one-use greenlight | Pass | `src/protocol/gateway-check-attempts.ts`, `greenlight_consumptions`, replay tests in `test/kernel.test.ts`, `test/package-install-gateway.test.ts` |
...
| no greenlight or gate pass while isolated | Pass | `src/protocol/policy.ts`, `src/protocol/gateway-check-attempts.ts`, isolation and breaker tests in `test/kernel.test.ts` and `test/d1-http.test.ts` |
...
| no receipt that blurs gate check and downstream execution | Pass | `Receipt` split statuses, `src/protocol/gateway-check-artifacts.ts`, pending/unknown reconciliation tests |
