...
| [0002: Generated Execution Graph Coverage Boundary](./0002-generated-execution-graph-coverage.md) | Accepted; locally implemented | Generated execution graph evidence can feed candidate extraction only when whole-block coverage is clean, issuer-bound, redacted, catalog/gateway-bound, and digest-bound. The graph is not authority. | v0.2.4 kernel slice implements graph record, compile refusal, graph/node candidate binding, proposal-time graph reload, durable graph drift refusal, catalog/registry binding refusal, codemode runtime graph production, and codemode whole-block partial-credit refusal. [Plan 03](../plans/03-plan-eng-review-generated-execution-graph-coverage.md) remains open only for any future public graph surface decision. | May be cited as current local kernel behavior. Do not claim arbitrary shell control, public graph API, or runtime plugin/provider enforcement. |
...
| [0004: Pre-Contract Refusal Evidence Boundary](./0004-pre-contract-refusal-evidence-boundary.md) | Implemented locally | Pre-contract generated-execution coverage failure is compiler/graph refusal evidence only. The existing `ProofGap` lifecycle starts after an exact `ActionContract` exists. | Implemented through [Plan 03](../plans/03-plan-eng-review-generated-execution-graph-coverage.md) refusal tests. | May be cited as current local evidence taxonomy. Do not claim public graph evidence APIs. |
...
ADR 0002 is the active local generated-graph coverage ADR. ADR 0003 is the
implemented architecture-style ADR. ADR 0004 is the implemented local evidence
taxonomy ADR that narrows ADR 0002 for pre-contract coverage failures. ADR 0005 is the
