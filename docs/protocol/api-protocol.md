...
Protocol-spec alignment is tracked in
[`02c-plan-eng-review-protocol-spec-alignment.md`](../plans/02c-plan-eng-review-protocol-spec-alignment.md).
The transition boundary now requires `X-Handshake-Protocol-Version` and an
opaque `X-Handshake-Request-Identity`, echoes the request identity on accepted
transitions, and records committed request-context digests. Optional
`X-Handshake-Originating-Identity` is accepted only as a `sha256:` digest or
opaque `ref:` value. Operation claims and raw request context remain internal;
only transition headers, refusal codes, receipts, proof gaps, and redacted
exports are exposed.
...
`orphanIsolationRequested` is explicit lifecycle configuration; `evidenceRefs`
can describe why isolation was requested, but evidence refs do not control
whether the kernel creates isolation.
