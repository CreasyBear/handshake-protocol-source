...
The protocol kernel is the source-owned authority state machine and schemas for exact contracts, policy decisions, one-use greenlights, gateway checks, receipts, refusals, proof gaps, isolation, and terminal certificates.

Product surfaces are projection/readback surfaces such as CLI, MCP, docs,
demos, and service-facing readbacks that expose proposal/evidence/readback
without creating authority. Role-scoped protocol transition clients, including
SDK policy and gateway clients, transport specific kernel transitions under
custody; they are not product authority surfaces. A cleared protected-action
event is a specific terminal Handshake event with reconstructable evidence. The
...
| `src/surfaces/`       | Source-owned boundary manifests, service workflow projection maps, and shared non-authority outcomes for product projection/readback surfaces. | Protocol meaning, policy interpretation, route behavior, gateway authority, reusable auth, or public product claims. |
